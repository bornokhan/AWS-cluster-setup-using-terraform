resource "aws_eip" "aws_cluster_bastion_eip" {
  domain = "vpc"
  tags = {
    Name = "aws_cluster_bastion_eip"
    Type = "IPV4"
  }
}
...
resource "aws_instance" "bastion" {
  ...
  root_block_device {
    ...
    iops                  = 5000
    tags = {
      Name = "aws_cluster_bastion_root_blk_ebs"
    }
  }
  tags = {
    Name = "bastion",
  ...
  user_data = <<-EOF
              ...
              sudo apt update
              sudo apt upgrade -y
              echo "10.242.110.5      master-node" >> /etc/hosts
              echo "10.242.120.5      workernode1" >> /etc/hosts
              echo "10.242.130.5      workernode2" >> /etc/hosts
              echo '${base64encode(file("./ssh/aws_cluster_key"))}' | base64 --decode > /home/ubuntu/.ssh/id_rsa
...
resource "aws_eip_association" "aws_cluster_bastion_eip_association" {
  instance_id   = aws_instance.bastion.id
  allocation_id = aws_eip.aws_cluster_bastion_eip.id
}